...
var imgGallerySwiper;

function imgGallerySwiper() {
	imgGallerySwiper = new Swiper(".gallerySwiper", {
		slidesPerView: 'auto',
		spaceBetween: 30,
		loop: true,
		navigation: {
...
function updateProgress(swiper) {
	...
	const virtualIndex = swiper.realIndex; // 복제까지 포함
	const actualSlides = $('.swiper-slide:not(.swiper-slide-duplicate)').length; // 실제 슬라이드 수 
	const progress = ((virtualIndex + 1) / (actualSlides)) * 100;
...
function galleryPOP(toggle, type, index) { // 갤러리팝업 열고,닫기
	...
	if(type === "type4") return galleryType4(toggle, index);
}
...
function galleryType4(toggle, index){ // 원형 갤러리
	var el = $(".gallery-pop-wrap");
	...
	if(toggle === "open"){
		...
		var index = $('#pop-gallery li.active').index();
		console.log(index);		
		imgGallerySwiper.slideToLoop(index-1);
		el.removeAttr("style");
		el.find(".pop-body").removeAttr("style");
		scrollAuto();
	}
